...
This repository is built in reviewable slices. **PR-1 is deliverable one only.**
...
tests/                 Unit tests for the layout guarantees and validation rules
web/                   React console (PR-1: gateway harness)
docker/                Dockerfiles, nginx and Mosquitto config
```

## The three decisions this PR is really about
...
wall of alerts — the correlated-disconnect requirement from the research report. The
`CorrelatedOffline` device status exists for it; the correlation pass that sets it lands
in PR-3.
